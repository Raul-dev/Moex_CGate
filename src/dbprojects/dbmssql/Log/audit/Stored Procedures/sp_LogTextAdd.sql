...
EXEC [audit].[sp_LogTextAdd]    @AuditEnable ='FullAuditEnabled', @Message ='test'
...
CREATE   PROCEDURE [audit].[sp_LogTextAdd]   
...
BEGIN
    ...
    IF @AuditTypeID = 2
        EXEC [$(LinkSRVLog)].[$(DatabaseName)].[audit].sp_LnkLtInsert
            @ObjectId         = @ObjectId,
            @KeyField         = @KeyField,
            @KeyValue         = @KeyValue,
            @MessageCode      = @MessageCode,
